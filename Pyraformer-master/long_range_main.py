...
def eval_epoch(model, test_dataset, test_loader, opt, epoch,run_name):
...
def train(model, optimizer, scheduler, opt, model_save_dir):
    ...
    for epoch_i in range(opt.epoch):
        ...
        mse, mae, rmse, mape, mspe = eval_epoch(model, test_dataset, test_dataloader, opt, epoch_i,opt.run_name)
...
def evaluate(model, opt, model_save_dir):
    ...
    best_metrics = []
    mse, mae, rmse, mape, mspe = eval_epoch(model, test_dataset, test_dataloader, opt, 0,opt.run_name)
...
def parse_args():
    ...
    parser.add_argument('-input_size', type=int, default=7) #168
    parser.add_argument('-predict_step', type=int, default=24)
    parser.add_argument('-inverse', action='store_true', help='denormalize output data', default=True)
    ...
    parser.add_argument('-iter_num', type=int, default=2) # Repeat number.
